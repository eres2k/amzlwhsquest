...
export class AssetGenerator {
    ...
    generateEnvironmentAssets() {
        ...
        this.assets.environment.wall = this.createCanvas(16, 16, ctx => {
            // Metal shelf frame (orange Amazon-style racking)
        ...
        this.assets.environment.conveyor = this.createCanvas(16, 16, ctx => {
            ...
            // Belt texture/rollers
            ctx.fillStyle = '#4b5563';
            for (let i = 0; i < 4; i++) {
                ctx.fillRect(2 + i * 4, 4, 2, 8);
            }

            // Direction arrows (Amazon yellow)
        ...
        // Generate warehouse-specific tiles
        this.generateWarehouseTiles();

        // Generate Amazon branding signs
        this.generateWarehouseSigns();
    }

    /**
     * Generate warehouse-specific tiles
     */
    generateWarehouseTiles() {
        // Floor with safety markings (yellow lines)
        ...
        // Shelf with boxes (variant 1 - cardboard boxes)
        ...
        // Shelf with boxes (variant 2 - totes/bins)
    ...
    /**
     * Generate Amazon warehouse signs
     */
    generateWarehouseSigns() {
        // DVI1 Sign (Austria warehouse code)
        ...
        this.assets.environment.signAustria = this.createCanvas(32, 16, ctx => {
            // Sign background (Austrian flag colors)
        ...
        // Fire exit sign
        this.assets.environment.signExit = this.createCanvas(24, 12, ctx => {
            ctx.fillStyle = '#15803d';
            ctx.fillRect(0, 0, 24, 12);
            ctx.fillStyle = '#22c55e';
            ctx.fillRect(1, 1, 22, 10);

            // Running person icon
            ctx.fillStyle = '#fff';
            ctx.fillRect(3, 3, 2, 4); // Body
            ctx.fillRect(2, 2, 2, 2); // Head
            ctx.fillRect(4, 5, 2, 1); // Arm
            ctx.fillRect(2, 6, 2, 2); // Leg
            ctx.fillRect(4, 7, 1, 2); // Leg
            ...
            ctx.fillText('EXIT', 16, 9);
        });

        // Aisle markers (A1, A2, B1, etc.)
